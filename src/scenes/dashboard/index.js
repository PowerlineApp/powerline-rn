...
const FooterTabButton = ({ badge = 0, active, onPress, icon, title }) => {
  ...
  const BudgeButton = ({ children }) => (
    <Button badge {...buttonProps}>
      {children}
    </Button>
  );
  ...
  return (
    <View style={[styles.containerTabButton, styles.borderTop]}>{content}</View>
  );
};
...
class Home extends Component {
              ...
              { text: "Verify", onPress: () => Actions.verifyProfile() },
              {
                ...
                onPress: () => {
                  ...
                  try {
                    ...
                    console.warn(
                      "couldnt set notification to verify later",
                      error
                    );
                  }
                }
    ...
    switch (type) {
      case "join-to-group-approved":
        Actions.groupprofile({
          id: notification.additionalData.entity.target.id
        });
        break;
      case "follow-request":
  ...
  _openComment(token, data) {
    ...
    let comment = getComments(token, target.type, target.id).then(res => {
      Actions.commentDetail({
        comment: (res.comments || []).find(
          comment => comment.id === target.comment_id
        ),
  ...
  _approveFollowRequest(token, data, dispatch) {
    acceptFollowers(
      token,
      data.notification.payload.additionalData.entity.target.id
    )
      .then(ret => {
        dispatch({ type: "RESET_FOLLOWERS" });
        getFollowers(token, 1, 20)
          .then(ret1 => {
    ...
    switch (actionButton) {
      ...
      case "reply-comment-button":
        return this.redirect(type, data.notification.payload, {
          commenting: true,
          commentText:
            "@" + data.notification.payload.additionalData.user.username
        });
        break;
  ...
  async selectGroup(group) {
    ...
    if (group == "all") {
      ...
      // return;
      let groupObj = this.props.groupList.find(
        groupObj => groupObj.group_type_label === group
      );
    ...
    let options = [
      <MenuOption key="petition" value={"petition"}>
        <Button
          ...
          dark
          onPress={() => {
            this.selectNewItem("petition");
            Mixpanel.track("Opened New User Petition Form");
          }}
        >
      ...
      <MenuOption key="post" value={"post"}>
        <Button
          ...
          dark
          onPress={() => {
            this.selectNewItem("post");
            Mixpanel.track("Opened New Post Form");
          }}
        >
      ...
      options.unshift(
        ...
        <MenuOption key="group_fundraiser" value={"group_fundraiser"}>
          <Button
            ...
            dark
            onPress={() => {
              this.selectNewItem("group_fundraiser");
              Mixpanel.track("Opened New Fundraiser Form");
            }}
          >
        ...
        <MenuOption key="group_event" value={"group_event"}>
          <Button
            ...
            dark
            onPress={() => {
              this.selectNewItem("group_event");
              Mixpanel.track("Opened New Event Form");
            }}
          >
        ...
        <MenuOption key="group_petition" value={"group_petition"}>
          <Button
            ...
            dark
            onPress={() => {
              this.selectNewItem("group_petition");
              Mixpanel.track("Opened New Group Petition Form");
            }}
          >
            <Icon name="ios-clipboard" style={styles.menuIcon} />
        ...
        <MenuOption key="group_discussion" value={"group_discussion"}>
          <Button
            ...
            dark
            onPress={() => {
              this.selectNewItem("group_discussion");
              Mixpanel.track("Opened New Discussion Form");
            }}
          >
        ...
        <MenuOption key="group_poll" value={"group_poll"}>
          <Button
            ...
            dark
            onPress={() => {
              this.selectNewItem("group_poll");
              Mixpanel.track("Opened New Poll Form");
            }}
          >
  ...
  render() {
    ...
    return (
      <MenuContext customStyles={menuContextStyles}>
            ...
            <Grid>
              <Row>
                <Col style={styles.col}>
                  <Button
                    style={
                      selectedGroup.header == "all"
                        ? styles.iconActiveButton
                        : styles.iconButton
                    }
                    onPress={() => {
                      Keyboard.dismiss();
                      this.selectGroup("all");
                      Mixpanel.track("All Feed Loaded");
                    }}
                  >
                    <Image
                      style={styles.iconP}
                ...
                <Col style={styles.col}>
                  <Button
                    id="local"
                    style={
                      selectedGroup.header == "local"
                        ? styles.iconActiveButton
                        : styles.iconButton
                    }
                    onPress={() => {
                      Keyboard.dismiss();
                      this.selectGroup("local");
                      Mixpanel.track("Town Feed Loaded from Home");
                    }}
                  >
                ...
                <Col style={styles.col}>
                  <Button
                    ...
                    onPress={() => {
                      ...
                      Mixpanel.track("State Feed Loaded from Home");
                    }}
                  >
                    <Icon active name="pin" style={styles.icon} />
                  </Button>
                ...
                <Col style={styles.col}>
                  <Button
                    style={
                      selectedGroup.header == "country"
                        ? styles.iconActiveButton
                        : styles.iconButton
                    }
                    onPress={() => {
                      Keyboard.dismiss();
                      this.selectGroup("country");
                      Mixpanel.track("Country Feed Loaded from Home");
                    }}
                  >
                    <Icon active name="pin" style={styles.icon} />
                  </Button>
                ...
                <Col style={styles.col}>
                  <Button
                    style={
                      selectedGroup.header == "more"
                        ? styles.iconActiveButton
                        : styles.iconButton
                    }
                    onPress={this.goToGroupSelector}
                  >
        ...
        <PLOverlayLoader
          visible={this.state.loading || this.props.loadingActions}
          marginTop={200}
          logo
        />
          ...
          <FooterTab style={{ backgroundColor: "transparent" }}>
            ...
            <Button style={isIOS ? {} : { height: 75 }}>
              ...
              <Menu
                ...
                onSelect={value => this.selectNewItem(value)}
                ref={this.onRef}
              >
                ...
                <MenuOptions
                  ...
                  renderOptionsContainer={optionsRenderer}
                >
